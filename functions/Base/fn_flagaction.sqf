...
switch _typeX do
{
    ...
    case "seaport":
    {
		...
		_flag addAction ["Mandalorian Workshop", {execVM "functions\factory\openDialog.sqf"},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4];
    };
    case "garage":
    {
    ...
    case "SDKFlag":
    {
		...
		if !("seaport" in allMapMarkers) then {
			_flag addAction ["Mandalorian Workshop", {execVM "functions\factory\openDialog.sqf"},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])",4];
		};
        [_flag] call HR_GRG_fnc_initGarage;
    };
